...
class LlvmCodeGen {
 public:
  ...
  static int LoadImpalaIR(
      ObjectPool*, const std::string& id, unique_ptr<LlvmCodeGen> codegen);

  /// Load a pre-compiled IR module from 'file'.  This creates a top level
  /// codegen object.
  /// codegen will contain the created object on success.
  static int LoadFromFile(ObjectPool*, const std::string& file, const std::string& id,LlvmCodeGen **codegen);
  ...
  static int LoadFromMemory(ObjectPool*, llvm::MemoryBufferRef* module_ir,
  ...
  llvm::ConstantInt *getConstant(uint64_t bitsize, uint64_t value);
  ...
  class FnPrototype {
   public:
    ...
    /// the return type.
    FnPrototype(LlvmCodeGen*, const std::string& name, llvm::Type* ret_type, bool isinline);
  ...
  llvm::Type* GetType(const TypeKind &type);
  ...
  void RegisterExprFn(int64_t id, llvm::Function* function) {
    assert(registered_exprs_map_.find(id) == registered_exprs_map_.end());
  ...
  //TODO with a name
  uint64_t GetFunction(string functionname);

  bool hasFunction(string functionname){
	  return this->module_->getFunction(functionname)!=NULL;
  }
  ...
  llvm::Type *void_ptr_type(){llvm::PointerType::get(void_type_,0);}
  ...
  static unique_ptr<llvm::Module> LoadModuleFromFile(LlvmCodeGen* codegen, const string& file);
  ...
  static int LoadModuleFromMemory(LlvmCodeGen* codegen, llvm::MemoryBufferRef* module_ir,
  ...
  int LinkModule(const std::string& file);
...
private:
  ...
  unique_ptr<llvm::LLVMContext> context_;
  ...
  unique_ptr<llvm::ExecutionEngine> execution_engine_;
