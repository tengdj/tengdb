...
// Helper function
    ...
    struct Expr {
        ...
        void addColumn(std::map<std::string, bool>* colmap){
        	if(this->type==ExprType::kExprColumnRef){
        		(*colmap)[std::string(name)] = true;
        	}
        	if(expr){
        		expr->addColumn(colmap);
        	}
        	if(expr2){
        		expr2->addColumn(colmap);
        	}
        }
